...
Четвертый столбец - размер символа TODO ПОЧЕМУ РАЗМЕР `__reset_handle` РАВЕН 0. Последний столбец - это имя символа.
...
У вас может возникнуть вопрос. Мы загружаем секцию .text по адресу LMA, а в коде обращаемся к ней по VMA. И VMA не равен LMA! Тогда каким образом микроконтроллер корректно выполняет наш код? Все дело в механизме remap, про который вам расскажут на лекции.
