...
- [ ] Отрефакторить код:
	...
	- [ ] Инициализировать handler_ticks нулём и сделать так, чтобы код заработал. Для этого нужно аналогично секции .data скопировать секцию .bss. Придётся изменить и `entry.S`, и `entry.lds`.
