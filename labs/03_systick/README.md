...
## Таймеры
Таймер — это просто счетчик, который при достижении заданного значения может вызывать определённые события — прерывания. С помощью таймера можно измерять временя, генерировать ШИМ, управлять работой протоколов общения.
...
В работе нас будет интересовать SysTick timer. Инициализация таймера происходит в файле [systick.c](systick.c) в функции systick_init, а обработчик прерываний в функции systick_handler. Самостоятельно с помощью даташита разберитесь в том, какие регистры записываются в systick_init и к каким последствияем это приводит.
## Прерывания
...
В конце файла определен знакомый вам `.section .vector_table`. Как и в прошлой лабораторной работе в нем определены начальное значание стека и символ __reset_handler, но теперь определены в этой секции определены адреса функций обработчиков прерываний. Вот они, слева направо:
...
Их поряд не случаен, он описан в [Cortex-M0 Devices Generic User Guide](docs/cortex_m0_gug.pdf):
![imgs/vector_table.png](imgs/vector_table.png)

Итак, порядок обработки прерываний следующий: микроконтроллер принимает прерывание и определяет его тип; берет адрес функции из vector table; вызывает функцию. В нашем случае при возникновении прерывания SysTick микроконтроллер вызовет функцию, адрес которой лежит в 0x3C, то есть systick_handler.
...
Статическая переменная не может храниться в стеке, так как её значение сохраняется при выходе из стека, значит, её значение должно храниться в памяти микроконтроллера. Статическая переменная не может находиться во Flash-памяти, потому что эта память является неизменяемой, а в программе мы меняем handler_ticks. Значит, её нужно хранить в SRAM-памяти. Но SRAM-память очищается при выключении питания, остается только Flash-память...
...
- `. = ALIGN(4);` нужен, чтобы все адреса были кратны 4. TODO - а зачем?
...
```
Видно, что секция .data находится по адресу 0x20000000, а её размер равен 4 байтам, потому что размер handler_ticks - 4 байта. Попробуйте изменить тип handler_ticks в файле [systick.c](systick.c) с типа int на double. Какое значение теперь примет __data_end_vma? Попробуйте при инициализации присвоить handler_ticks значение = 0. Запустите программу. Что произошло?
...
- [ ] Инициализируйте handler_ticks нулём и сделайте так, чтобы код заработал. Для этого нужно аналогично секции .data скопировать секцию .bss.
- [ ] Устранить гонку при работе с диодами. ЧТО ЭТО???
- [ ] Перенести на прерывания: КАЖЕТСЯ СЛИШКОМ СЛОЖНО
